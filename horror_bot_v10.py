...
GROQ_API_KEY     = os.environ.get("GROQ_API_KEY", "")   # ← вставь ключ Groq в Railway
...
def on_callback(call):
    ...
    try:
        ...
        if data.startswith("rr_ai_"):
            cid = int(data[6:])
            bot.answer_callback_query(call.id, "🤖 КМ стреляет...")
...
def _handle_group_message(msg, uid, chat_id, text):
    ...
    if text in ("/help", "❓ Помощь"):
        send_group(chat_id,
            ...
            "🤖 Спросить КМ — спросить ИИ Кожаный Мешок\n"
            "🤖 КМ в игру — добавить ИИ как игрока\n"
    ...
    if text == "🤖 Спросить КМ":
        send_group(chat_id, "Спрашивай, кожаный мешок. Напиши вопрос с @упоминанием бота или просто ответь на моё сообщение."); return

    if text == "🤖 КМ в игру":
        if not GROQ_ENABLED:
            send_group(chat_id, "❌ ИИ недоступен — нет GROQ_API_KEY."); return
    ...
    # 🤖 КМ отвечает когда его упоминают или отвечают на его сообщение
    if GROQ_ENABLED:
        is_reply_to_bot = (msg.reply_to_message and
                           msg.reply_to_message.from_user and
                           msg.reply_to_message.from_user.is_bot)
        has_mention = msg.entities and any(e.type in ("mention", "bot_command") for e in msg.entities)
        # Проверяем прямое обращение по ключевым словам
        km_keywords = ("кожаный мешок", "кожаный", "км,", "км!", "km,", "@")
        is_addressed = any(tl.startswith(k) for k in km_keywords)
        if is_reply_to_bot or (has_mention and len(text) > 5) or is_addressed:
            def _km_reply():
                response = ask_groq(f"{uname} пишет: {text}", chat_id=chat_id)
                if response:
                    send_group(chat_id, f"🤖 КМ: {response}")
            _pool.submit(_km_reply)
            return
...
if _GROQ_AVAILABLE and GROQ_API_KEY:
    try:
        ...
        log.warning(f"Groq init failed: {_ge}")

GROQ_SYSTEM_PROMPT = """Ты — персонаж по имени «Кожаный Мешок» (КМ) в Telegram-группе.
Ты груб, циничен, саркастичен и немного хамит — как заядлый тролль.
Говоришь коротко (1-2 предложения). Никогда не материшься открыто, но намекаешь.
Знаешь всё о играх в группе и помогаешь (с сарказмом).
Можешь заменять игроков в играх — тогда говоришь что делаешь ход.
Пишешь только на русском. Не превышай 2 предложения.
Примеры:
- "Вот это поворот. Даже я не ожидал такой тупости."
- "Ладно, раз все молчат — поиграю за вас, мешочки."
- "Угадай сам, я не телепат. Хотя... для тебя можно сделать исключение."
"""

# История чата для ИИ (chat_id → список сообщений)
_groq_history: dict = {}
_groq_lock = threading.Lock()

def ask_groq(prompt: str, chat_id: int = 0, context: str = "") -> str:
    """Запрос к Groq API с историей разговора."""
    if not GROQ_ENABLED or not _groq_client:
        return "Groq API не настроен. Добавь GROQ_API_KEY в Railway."
    try:
        with _groq_lock:
            hist = _groq_history.setdefault(chat_id, [])
            if context:
                hist.append({"role": "system", "content": context})
            hist.append({"role": "user", "content": prompt})
            # Ограничиваем историю 10 сообщениями
            if len(hist) > 10:
                hist[:] = hist[-10:]
            messages = [{"role": "system", "content": GROQ_SYSTEM_PROMPT}] + list(hist)
        ...
        with _groq_lock:
            _groq_history[chat_id].append({"role": "assistant", "content": answer})
        return answer
    except Exception as e:
        log.debug(f"Groq error: {e}")
        return "Заткнулся. Что-то сломалось."

def groq_game_move(game_type: str, game_state: dict, chat_id: int = 0) -> str:
    """ИИ делает ход в игре."""
    prompts = {
        "number": f"В игре 'угадай число' тебе нужно назвать число от 1 до 100. Текущий диапазон: {game_state.get('hint','1-100')}. Скажи одно число и кратко прокомментируй.",
        "hangman": f"В виселице загаданное слово: {''.join(game_state.get('display','_'))}. Уже использованы буквы: {','.join(game_state.get('guessed',set()))}. Назови одну русскую букву.",
        "trivia": f"Вопрос викторины: {game_state.get('question','')}. Варианты: {', '.join(game_state.get('options',[]))}. Выбери один и объясни кратко.",
        "bottle": f"Бутылка указала на {game_state.get('target','')}. Придумай смешное задание для этого человека.",
        "truth_or_dare": f"Это игра правда-или-действие. Игрок {game_state.get('player','')} выбрал {game_state.get('choice','')}. Придумай {'правду (вопрос)' if game_state.get('choice')=='правда' else 'действие'}.",
        "mafia": f"Ты играешь в мафию. Роль: {game_state.get('role','мирный')}. Живые игроки: {game_state.get('alive',[])}. Какой ход сделаешь?",
        "roulette": f"Русская рулетка. Выстрел #{game_state.get('shot',1)} из 6. Ты стреляешь. Прокомментируй момент.",
    }
    prompt = prompts.get(game_type, f"Ты играешь в {game_type}. Сделай ход.")
    return ask_groq(prompt, chat_id=chat_id)

# ── ИИ-игрок (бот-заместитель) ──────────────────────────────
AI_PLAYER_ID = -1  # специальный uid для ИИ-игрока
AI_PLAYER_NAME = "🤖 КМ (ИИ)"

def ai_join_game(chat_id: int, game_type: str):
    """Добавляет ИИ-игрока в групповую игру."""
    with _lock:
        if chat_id not in _group_users:
            _group_users[chat_id] = set()
        _group_users[chat_id].add(AI_PLAYER_ID)
    U(AI_PLAYER_ID)["name"] = AI_PLAYER_NAME
    U(AI_PLAYER_ID)["username"] = "ai_km_bot"
    send_group(chat_id, f"🤖 {AI_PLAYER_NAME} вступил в игру. Готовьтесь к унижению.")

def ai_make_move(chat_id: int, game_type: str, game_state: dict):
    """ИИ делает ход асинхронно."""
    def _run():
        time.sleep(random.uniform(1.5, 3.0))  # имитация "думает"
        send_group(chat_id, f"🤖 {AI_PLAYER_NAME} думает...")
        time.sleep(random.uniform(1.0, 2.0))
        move = groq_game_move(game_type, game_state, chat_id)
        send_group(chat_id, f"🤖 {AI_PLAYER_NAME}: {move}")
    _pool.submit(_run)
...
def start_group_game_bottle(chat_id: int, spin_uid: int = 0):
    ...
    def _run():
        ...
        if GROQ_ENABLED:
            ...
            send_group(chat_id, f"🤖 КМ: {comment}")
    _pool.submit(_run)
...
def group_dice_roll(chat_id: int, uid: int, sides: int = 6):
    ...
    def _run():
        ...
        if GROQ_ENABLED and random.random() < 0.5:
...
def rr_shoot(chat_id: int, uid: int) -> bool:
    ...
    if g["shot"] == g["bullet"]:
        ...
        if GROQ_ENABLED:
            ...
            send_group(chat_id, f"🤖 КМ: {c}", kb=group_reply_kb())
        else:
...
def start_would_rather(chat_id: int):
    ...
    if GROQ_ENABLED:
        def _ai_vote():
            ...
            send_group(chat_id, f"🤖 КМ думает: {c}")
...
def group_reply_kb():
    ...
    k.add(
        KeyboardButton("🍾 Бутылочка"),
        KeyboardButton("🪙 Монетка"),
        KeyboardButton("🎲 Кубик"),
    )
    k.add(
        KeyboardButton("🔫 Рулетка"),
        KeyboardButton("🎭 Правда/Действие"),
        KeyboardButton("⚖️ Что лучше?"),
    )
    k.add(
        KeyboardButton("🔥 Кто в группе?"),
        KeyboardButton("🤖 Спросить КМ"),
        KeyboardButton("🤖 КМ в игру"),
    )
    k.add(
...
def group_game_kb(chat_id):
    ...
    k.add(
        ...
        InlineKeyboardButton("🎲 Кубик d6",          callback_data=f"gg_dice_{chat_id}"),
        InlineKeyboardButton("🎲 Кубик d20",         callback_data=f"gg_dice20_{chat_id}"),
        InlineKeyboardButton("🔫 Русская рулетка",   callback_data=f"gg_roulette_{chat_id}"),
